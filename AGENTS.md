...
## Core
- Repo: nastech-android. Two components: Kotlin bridge app (`nastech-android-bridge/`) + Python toolset (`tools/`, `tests/`, `nastech-android-plugin/`).
- Python prod copy lives in nastech-agent repo; this repo = standalone dev/test. APK does NOT depend on Python.
...
- Secrets: `~/.nastech/.env` (`ANDROID_BRIDGE_URL`, `ANDROID_BRIDGE_TOKEN`). Never echo/dump env. Never log pairing codes or tokens.
